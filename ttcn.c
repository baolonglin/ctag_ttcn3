...
const int ttcnKeywordCount = sizeof(ttcnKeywords)/sizeof(struct s_ttcnKeyword);
...
const int ttcnOpCount = sizeof(ttcnOps)/sizeof(struct s_ttcnOp);
...
static void findTTCNKeyword(ttcnToken_t * pTok) {
    int lower = 0, upper = ttcnKeywordCount-1, i, cmp;
    if(!pTok || !(pTok->value) || !(pTok->value->buffer)) {
        return;
    }
    /* Binary search */
    for(i=(lower+upper)/2; lower<=upper; i=(lower+upper)/2) {
        cmp = strcmp(pTok->value->buffer, ttcnKeywords[i].name);
        if(cmp == 0) {
            /* Gotcha! */
            pTok->type = ttcnKeywords[i].id;
            pTok->kind = ttcnKeywords[i].kind;
            return;
        }
        /* New search interval */
        else if(cmp > 0) lower = i+1;
        else upper = i-1;
    }
    /* Not found */
    pTok->type = T_ID;
...
ttcnToken_t * pTtcnToken = NULL;
int repeatLastToken = FALSE;

static void freeToken(ttcnToken_t * pTok) {
    if(pTok) {
        if(pTok->value) vStringDelete(pTok->value);
        free(pTok);
    }
}

/* This function skips all whitespace and comments */
int getNonWhiteSpaceChar (void) {
    int c, c2;
    while(1) {
        /* Skip whitespace */
        while (isspace(c = fileGetc()) && (c != EOF));
...
static ttcnToken_t * getToken (void) {
    ...
    !(pTtcnToken = (ttcnToken_t *)malloc(sizeof(ttcnToken_t)))) {
        return NULL;
    } else {
        pTtcnToken->type = 0;
    ...
    if (isalpha(c)) {
        ...
        while(isxdigit(c = fileGetc())) vStringPut(pTtcnToken->value, c);
        /* Must be terminated with "'O", "'B" or "'H" */
        if(c != '\'') pTtcnToken->type = 0;
        else {
            vStringPut(pTtcnToken->value, c);
            c = fileGetc();
            if((c != 'O') && (c != 'H') && (c != 'B')) pTtcnToken->type = 0;
            else vStringPut(pTtcnToken->value, c);
        }
    } else if (c=='"') {
        /* Charstring */
        pTtcnToken->type = T_LITERAL;
        ...
        pTtcnToken->value = vStringNew();
        /* Integer part */
        do vStringPut(pTtcnToken->value, c);
        while(isdigit(c = fileGetc()));
        /* Decimal dot */
        if(c == '.') {
            vStringPut(pTtcnToken->value, c);
            /* Fractional part */
            while(isdigit(c = fileGetc())) vStringPut(pTtcnToken->value, c);
        }
        /* Exponent */
        if((c == 'E') || (c == 'e')) {
            vStringPut(pTtcnToken->value, c);
            /* Exponent sign */
            if((c = fileGetc()) == '-') vStringPut(pTtcnToken->value, c);
            else fileUngetc(c);
...
static ttcnToken_t * matchToken (ttcnTokenType_t toktype) {
    ...
    if(pTok && (pTok->type == toktype)) return pTok;
    else {
        ungetToken();
        return NULL;
    }
...
static int matchBrackets (const char * br) {
    if(matchToken(br[0])) {
        int brcount = 1;
        while(brcount > 0) {
            if(matchToken(br[0])) brcount++;    /* Open */
            else if(matchToken(br[1])) brcount--;   /* Close */
            else if(!getToken()) return 0;  /* EOF */
        }
        return 1;
    } else return 0;
}
...
const ttcnTokenType_t ttcnExprOps[] = {
...
const int ttcnExprOpCount = sizeof(ttcnExprOps)/sizeof(ttcnTokenType_t);
...
static int matchExprOperator (void) {
    ...
    for(i=0;i<ttcnExprOpCount;i++)
        if(pTok->type == ttcnExprOps[i]) return 1;
    /* Not found */
    ungetToken();
    return 0;
}
...
static int parseExprOperand (void) {
    ttcnToken_t * pTok;
    if(matchBrackets(BR_PAR)) return 1; /* Nested expression in brackets */
    if(!(pTok = getToken())) return 0;
    switch (pTok->type) {
        case T_CREATE:
            /* Create statement */
...
static int parseSimpleExpression(void) {
    ...
    while(parseExprOperand() && matchExprOperator()) while(matchExprOperator());
    return 1;
}
...
static void makeEmptyControlTag()
{
    vString * control = vStringNewInit("control");
    makeSimpleTag(control, ttcnKinds, K_CONTROL);
}
...
static int parseArrayDef (void) {
    int res = 0;
    while(matchBrackets(BR_SQ)) res = 1;
    return res;
}
...
static int parseInitializer (void) {
    ...
    if(matchToken(T_OP_ASS)) {
        /* Compound expression */
        if(matchBrackets(BR_CUR)) return 1;
        /* Simple expression */
...
static int parseSignature (void) {
    return  (matchToken(T_SIGNATURE) && matchToken(T_ID) &&
            (!matchToken('.') || matchToken(T_ID)));
}
...
static int parseNestedTypeDef (void) {
    ...
    case T_SET:
        ...
        parseStringLength();
        /* RecordOf/SetOf */
        if(matchToken(T_OF)) return (parseType() || parseNestedTypeDef());
...
static int parseTypeDefBody (void) {
    ...
    case T_UNION:
        /* Parse Record/Set/Union structure */
        if(!(parseID(K_TYPE) || matchToken(T_ADDRESS))) return 0;
    ...
    while(pTok = getToken()) {
        ...
        switch(pTok->type) {
            ...
            case T_TYPE:    /* A.1.6.1.1 */
                ...
                break;
            case T_TEMPLATE:
                /*  A.1.6.1.3   TemplateDef ::= "template" (Type | Signature)
                    ID ... */
                (parseType() || parseSignature()) && parseID(K_TEMPLATE);
                break;
            case T_MODULEPAR:
                ...
                if(matchToken('{')) {
                    ...
                    /*  Type ModuleParList */
                    parseType() && parseNameInitList(K_MODULEPAR);
                }
                break;
            case T_IMPORT:
                ...
                if(!matchToken(T_FROM) || ! matchToken(T_ID)) break;
                matchToken(T_RECURSIVE);
                if(matchToken(T_ALL) && !matchToken(T_EXCEPT)) break;
...
extern parserDefinition* TTCNParser (void) {
    static const char *const extensions [] = { "ttcn", NULL };
    parserDefinition* def = parserNew ("TTCN");
    def->kinds      = ttcnKinds;
    def->kindCount  = KIND_COUNT(ttcnKinds);
